...
export class EntryFormRow extends React.Component<EntryFormRowProps, EntryFormRowState> {
  ...
  static validateForm = ({ value , description}) => {
    const errors = {};

    if (!value) {
      errors._error = 'Debés ingresar un saldo';
    }

    
    if (!description) {
      errors._error = 'Debés ingresar una descripción';
    }
      ...
      <tr className={styles.entryFormRow}>
        <td>
          ...
          >
            ...
            <div className={styles.formSection}>
              <Field component="input" name="description" type="text" placeholder="Descripción" />
            </div>
            <div className={styles.formSection}>
              <Field
                component="input"
